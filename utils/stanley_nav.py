...
class Robot:
    ...
    def describe(self):
        return tuple_to_string(self.get_position()) + ', ' + tuple_to_string(self.get_destination()) + ', ' +  str(self.__radius) + '\n'

def test():
    robot = Robot((1,2.5), (6,7), 2)
    #print((scenario.describe()))
    plt.cla()
    # for stopping simulation with the esc key.
    start = robot.get_position()
    end = robot.get_destination()

    if False:
        plt.scatter(start[0],start[1], s=160, c='forestgreen', marker=(5, 1))  #creates plot with star marker
